import type { DailyForecast, FetchLike } from "./types.ts";
...
export async function fetchDailyForecast(
  ...
  const res = await fetchImpl(url);
  if (!res.ok) {
    throw new Error(`Pronóstico de 7 días falló con estado ${res.status}`);
  }
  const data: unknown = await res.json();
  if (typeof data !== "object" || data === null) {
    throw new Error("Respuesta de pronóstico diario inválida");
  }
  const daily = (data as { daily?: unknown }).daily;
  if (typeof daily !== "object" || daily === null) {
    throw new Error("Respuesta de pronóstico diario sin datos");
  }
  ...
  if (
    ...
    !Array.isArray(values.temperature_2m_min) ||
    !Array.isArray(values.temperature_2m_max) ||
    values.time.length === 0 ||
    values.time.length !== values.temperature_2m_min.length ||
    values.time.length !== values.temperature_2m_max.length
  ) {
  ...
  return dates.map((date, index) => {
    ...
    if (
      ...
      throw new Error("Respuesta de pronóstico diario con datos inválidos");
    }
